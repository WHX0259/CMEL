...
class ClinicalImageBaseClusterPlusGatingModel(nn.Module):
    def __init__(self, cardinality=8, depth=29, nlabels=2, alpha=0.5, momentum =0.998, base_width=64, widen_factor=4, queue_size=48, text_dim=25, feat_dim=128, num_experts=3,num_iterations=1,cluster_init_type="kmeans",k=8):
        super(ClinicalImageBaseClusterPlusGatingModel, self).__init__()
        ...
        self.cluster_centers = torch.zeros(num_experts, feat_dim).detach()
    ...
    def calculateDensityWeight(self, features):
        ...
        return weights
    def kmeans_plus_plus_init(self):#max_iters是进行kmeans的次数
        merge_feature = self.merge_feat_queues()
        centers = [merge_feature[torch.randint(0, merge_feature.size(0), (1,)).item()]]
        #for each data point x, compute D(X), which is the distance between x and the nearst center that has already been chosen.
        for _ in range(1, self.n_clusters):
            # Compute distances from points to the closest center
            distances = torch.tensor([min([(x - c).norm().pow(2) for c in centers]) for x in merge_feature])
        ...
        # Convert list of centers to a torch tensor
...
#, cardinality=8, depth=29, nlabels=2ClinicalImageALBEFClusterPlusGatingModel, alpha=0.5, momentum =0.998, base_width=64, widen_factor=4, queue_size=48, text_dim=25, feat_dim=128, num_experts=3,num_iterations=1,cluster_init_type="kmeans",k=8
class ClinicalImageALBEFClusterDistancePlusGatingModel(nn.Module):
    def __init__(self, cardinality=8, depth=29, nlabels=2, alpha=0.5, momentum =0.998, base_width=64, widen_factor=4, queue_size=48, feat_dim=128, num_experts=3,num_iterations=5,cluster_init_type="kmeans++"):
        super(ClinicalImageALBEFClusterDistancePlusGatingModel, self).__init__()

        # 图像和临床特征提取
        self.encoder = ResNeXt_encoder(cardinality=cardinality, depth=depth, nlabels=nlabels, base_width=base_width, widen_factor=widen_factor)
        self.clinical_fc = nn.Sequential(
            nn.Linear(25, 128), nn.ReLU(), nn.Dropout(0.3),
        ...
        # 动量模型
        self.encoder_m = ResNeXt_encoder(cardinality=cardinality, depth=depth, nlabels=nlabels, base_width=base_width, widen_factor=widen_factor)
        self.clinical_fc_m = nn.Sequential(
            nn.Linear(25, 128), nn.ReLU(), nn.Dropout(0.3),
            nn.Linear(128, 64), nn.ReLU(), nn.Dropout(0.3)
        )
        
        self.attention = nn.ModuleList([Attention_block(self.stages[idx], self.stages[-1], 128) for idx in index])
        self.attention_m = nn.ModuleList([Attention_block(self.stages[idx], self.stages[-1], 128) for idx in index])


        self.cross_attention = nn.ModuleList([CrossModalAttention(128, 64, feat_dim) for _ in range(num_experts)])
        self.cross_attention_m = nn.ModuleList([CrossModalAttention(128, 64, feat_dim) for _ in range(num_experts)])
        ...
        self.gating = GatingNetwork(num_experts*feat_dim,num_experts=num_experts)
        #多少次更新聚类中心
        # 定义损失函数，例如交叉熵损失
        self.criterion = nn.CrossEntropyLoss()
        # 添加一个属性来跟踪迭代次数
        ...
        self.queue_size = queue_size
        self.n_clusters = num_experts
        self.register_buffer("feat_queues", torch.zeros(num_experts, feat_dim*num_experts, queue_size))
        self.feat_queues = nn.functional.normalize(self.feat_queues, dim=1)
        self.register_buffer("queue_ptrs", torch.zeros(num_experts, dtype=torch.long))
        self.cluster_centers = nn.Parameter(torch.zeros(num_experts, feat_dim*num_experts))
        self.if_init = False
    ...
    def kmeans_iteration(self):
        ...
        print("KMeans iterations complete.")
    def extract_and_align_features(self, image, clinical_data, encoder, attention, cross_attention):
        # 提取图像特征并融合
        img_feature_list = encoder(image)
        index = generate_attention_sequence(self.num_experts, len(img_feature_list))
        feat_last = img_feature_list[-1]

        attentioned_features = [attention[idx](img_feature_list[index[idx]], feat_last) for idx in range(len(index))]
        all_features =  attentioned_features
        img_features_list = [F.avg_pool2d(feat, feat.size()[3]).view(feat.size(0), -1) for feat in all_features]

        # 提取临床特征
        clinical_features = self.clinical_fc(clinical_data)

        # Cross-modal Attention 对齐
        aligned_features = []
        for idx in range(self.num_experts):
            aligned_feat = cross_attention[idx](img_features_list[idx], clinical_features)
            aligned_features.append(aligned_feat)
        
        return aligned_features

    def momentum_forward(self, image, clinical_data):
        return self.extract_and_align_features(image, clinical_data, self.encoder_m, self.attention_m, self.cross_attention_m)
    ...
    def forward(self, image, clinical_data):
        # 提取特征并对齐
        #4,128
        features = self.extract_and_align_features(image, clinical_data, self.encoder, self.attention, self.cross_attention)
        features = torch.stack(features, dim=1)#B,N,feature_dim的形状
        features_flatten = features.view(-1, self.feat_dim * self.num_experts)
        features_flatten = nn.functional.normalize(features_flatten, dim=1)
        with torch.no_grad():
            self._momentum_update()
            features_m = self.momentum_forward(image, clinical_data)
            features_m = torch.stack(features_m, dim=1)
            features_m_flatten = features_m.view(-1, self.feat_dim * self.num_experts)
            features_m_flatten = nn.functional.normalize(features_m_flatten, dim=1)#16,384#都归一化
            if self.if_init == False:
                bs = features_m.size(0)
                pseudo_labels_m = random.choices([0, 1, 2], k=bs)
                self.update_queues(features_m_flatten, pseudo_labels_m)
                self.init_itr +=1
                if self.init_itr > self.queue_size*self.num_experts / bs:
                ...
                sim_feat = features_flatten @ all_features / temp
                loss_sim = -torch.sum(F.log_softmax(sim_feat, dim=1) * sim_target, dim=1).mean()
                # 更新队列和聚类中心
                self.update_queues(features_m_flatten, pseudo_labels_m)
                self.iteration_counter += 1
                if self.iteration_counter % self.update_frequency == 0:
        ...
        outputs = [self.fusion_fc_list[idx](features[:,idx,:]) for idx in range(self.num_experts)]
        outputs_tensor = torch.stack(outputs, dim=1)
        # final_output = torch.mean(torch.stack(outputs, dim=0), dim=0)
        pseudo_labels_m_tensor = torch.tensor(pseudo_labels_m, dtype=torch.long).to(outputs_tensor.device)
...
# # 初始化模型
# model = ClinicalImageALBEFClusterDistancePlusGatingModel(cardinality=8, depth=29, nlabels=2, alpha=0.5, momentum =0.998, base_width=64, widen_factor=4, queue_size=4, feat_dim=128, num_experts=3,num_iterations=20,cluster_init_type="kmeans++")
